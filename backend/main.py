...
<head>
  ...
  <style>
    ...
    .card {{ background: #fff; border-radius: 1rem; box-shadow: 0 2px 8px rgba(0,0,0,.08); padding: 2rem; }}
    .badge {{ display: inline-block; background: #e0e7ff; color: #3730a3; font-size: .75rem;
              font-weight: 700; padding: .25rem .75rem; border-radius: 9999px; margin-bottom: 1rem; }}
...
async def forgot_password(req: ForgotPasswordRequest):
    ...
    return JSONResponse(status_code=200, content={"message": "If that email is registered, a reset link has been sent."})
...
async def change_password(
    ...
    Google OAuth users cannot use this endpoint (they manage passwords via Google).
    """
...
async def transcribe_audio(
    ...
    try:
        ...
        safe_stem = "".join(
            c if c.isalnum() or c in "-_" else "_" for c in original_stem
        ...
        transcript_data = diarize_and_transcribe(wav_path, language=transcribe_lang)
        transcript_txt = json_to_txt(transcript_data)
